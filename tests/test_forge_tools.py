...
class ForgeToolValidationTests(unittest.TestCase):
    @acceptance("AGENT-TOOLS-001@13", "surface")
    ...
    def test_list_recent_generations_arguments_are_validated(self):
        self.assertEqual({}, validate_forge_tool_request("list_recent_generations", {}))
        self.assertEqual(
            {"limit": 5, "include_grids": True},
            validate_forge_tool_request("list_recent_generations", {"limit": 5, "include_grids": True}),
        )
        with self.assertRaisesRegex(ForgeToolValidationError, "unsupported fields"):
            validate_forge_tool_request("list_recent_generations", {"filename": "x"})
        with self.assertRaisesRegex(ForgeToolValidationError, "limit must be an integer"):
            validate_forge_tool_request("list_recent_generations", {"limit": 0})
        with self.assertRaisesRegex(ForgeToolValidationError, "limit must be an integer"):
            validate_forge_tool_request("list_recent_generations", {"limit": 21})
        with self.assertRaisesRegex(ForgeToolValidationError, "include_grids must be a boolean"):
            validate_forge_tool_request("list_recent_generations", {"include_grids": "yes"})
        with self.assertRaisesRegex(ForgeToolValidationError, "target"):
            validate_forge_tool_request("list_recent_generations", {"target": "nope"})

    def test_read_pnginfo_arguments_are_validated(self):
        self.assertEqual(
            {"image_id": "gen-2-1"},
...
class ForgeToolApiTests(unittest.TestCase):
    ...
    @acceptance("AGENT-TOOLS-001@13", "revalidation,freshness")
