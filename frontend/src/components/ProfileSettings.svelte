<script lang="ts">
  ...
  import { Activity, Brain, Check, Copy, Grip, KeyRound, Languages, MoreHorizontal, Plus, RefreshCw, RotateCcw, Save, ShieldAlert, Trash2, X } from "lucide-svelte";
  ...
  type ConnectionMode = "openai-compatible" | "gemini-native";
  const connectionMode = $derived<ConnectionMode>(selected.protocol === "gemini-native" ? "gemini-native" : "openai-compatible");
  ...
  async function testConnection(): Promise<void> {
    ...
    const timeout = Math.min(30, Math.max(1, selected.parameters.timeout));
    let timedOut = false;
    ...
    <header class="pa-profile-window-header" use:pointerWindow={{ mode: "drag", layout: () => layout, update: updateLayout, minimum, interacting: (active) => interacting = active }}>
      ...
      <div class="pa-profile-window-actions">
        ...
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Portal><DropdownMenu.Content class="pa-dropdown-content"><DropdownMenu.Item class="pa-dropdown-item" onclick={() => $useUiStore.setAgentGeneration(!$useUiStore.agentGeneration)}>{#if $useUiStore.agentGeneration}<Check size={13} />{:else}<span class="pa-dropdown-check-spacer"></span>{/if}{$useUiStore.agentGeneration ? t("profiles.agent_generation.on", "Agent image generation: on") : t("profiles.agent_generation.off", "Agent image generation: off")}</DropdownMenu.Item><DropdownMenu.Item class="pa-dropdown-item" onclick={() => $useUiStore.resetWindowLayouts()}><RotateCcw size={13} />{t("profiles.interface.reset", "Reset window layouts")}</DropdownMenu.Item><DropdownMenu.Item class="pa-dropdown-item pa-profile-danger-text" onclick={() => confirm = "restore"}><RotateCcw size={13} />{t("profiles.restore", "Restore defaults")}</DropdownMenu.Item></DropdownMenu.Content></DropdownMenu.Portal>
        </DropdownMenu.Root>
        <DropdownMenu.Root>
        ...
        <Tabs.Root bind:value={tab} class="pa-profile-tabs"><Tabs.List class="pa-profile-tabs-list" aria-label={t("profiles.advanced_tabs", "Profile settings")}>{#each profileTabs as item}<Tabs.Trigger value={item[0]} class="pa-profile-tab">{item[1]}</Tabs.Trigger>{/each}</Tabs.List>
            ...
            <Tabs.Content value="advanced"><div class="pa-profile-tab-content">
              ...
              <More label={t("profiles.more.advanced", "Advanced settings")}>
                <div class="pa-profile-route-grid"><Route label={t("profiles.active_profile", "Active profile")} value={$useProfileStore.activeProfileId} profiles={enabledProfiles} onchange={(id: string) => $useProfileStore.activateProfile(id)} /></div>
