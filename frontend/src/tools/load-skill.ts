...
export const PROMPT_SKILL_NAMES = ["danbooru_tags", "anima_dit", "forge_couple", "krea2"] as const;
...
function textResult(details: Record<string, unknown>): AgentToolResult<unknown> {
  const displayDetails = Object.fromEntries(
    ["ok", "name", "title"].filter((key) => details[key] !== undefined).map((key) => [key, details[key]]),
  );
  return { content: [{ type: "text", text: JSON.stringify(details) }], details: displayDetails };
}
...
export function createLoadSkillTool(options: LoadSkillFactoryOptions = {}): AgentTool<typeof LOAD_SKILL_SCHEMA, unknown> & { permission: "read" } {
  return {
    ...
    description: [
      ...
      "krea2 for Krea 2 natural-language prompting, text rendering, and Turbo guidance;",
