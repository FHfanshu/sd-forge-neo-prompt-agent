...
import { CORE_TOOL_NAMES } from "./tool-groups";
...
export class PromptAgentToolRegistry {
  ...
  core(): AgentTool<any>[] {
    ...
    return this.list().filter((tool) => core.has(tool.name));
  }
...
export function createForgeToolRegistry(options: ForgeToolFactoryOptions = {}): PromptAgentToolRegistry {
  ...
  registry.register(createLoadToolsTool({ resolveTools: () => registry.list() }));
  return registry;
}
