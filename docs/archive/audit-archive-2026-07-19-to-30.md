# Audit Archive: 2026-07-19 to 2026-07-30

Entries moved from AUDIT.md to keep it under the 1000-line source limit.
...
## 2026-07-30 Clipboard Image Attachments
- Root cause: the composer accepted picker and dropped files but had no paste
  handler, so clipboard images never reached the existing attachment pipeline.
- Added image extraction from clipboard items/files in `Surface.svelte` and
  routed pasted images through the same count, per-file size, total-size, and
  data-URL validation as picker and drop attachments. Image paste prevents the
  native event only when images are present; ordinary text paste stays native.
- Advanced `IMAGE-INPUT-001` to revision 2 with a `clipboard-paste` scenario and
  added a regression that failed before implementation and passed afterwards.
- Svelte check completed with 0 errors and 0 warnings, the production bundle
  rebuilt successfully, and the affected gate passed 38 Python tests, 122
  frontend tests, and all 7 browser scenarios. The full gate passed 106 Python
  tests, 13 host contracts, 195 frontend tests, build/budget, 7 Playwright
  scenarios, and browser-script syntax.
- Live local and FRP checks attached a clipboard PNG, left text-only paste
  unblocked, and reported no console errors. A controlled restart changed PID
  53196 to 17240; post-restart FRP loaded the new bundle and repeated the result.
